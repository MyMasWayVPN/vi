...
function LOGO() {
 ...
 ───│    $Green├─┤│ │ │ │ │└─┐│  ├┬┘│├─┘ │   │  │ │ ├┤ $NC   │───
 ───│    $Green┴ ┴└─┘ ┴ └─┘└─┘└─┘┴└─┴┴   ┴   ┴─┘┴ ┴ └─┘$NC   │───
    │    ${YELLOW}Copyright${FONT} (C)$GRAY https://github.com/mymaswayvpn$NC     │
    └───────────────────────────────────────────────┘
...
data_ip="https://raw.githubusercontent.com/MyMasWayVPN/mymaswayvpn.github.io/main/akses/ip/buyer/izin"
data_ip="https://raw.githubusercontent.com/MyMasWayVPN/mymaswayvpn.github.io/main/akses/ip/buyer/izin"
checking_sc() {
...
Exp1=$(curl https://raw.githubusercontent.com/MyMasWayVPN/mymaswayvpn.github.io/main/akses/ip/buyer/izin | grep $MYIP | awk '{print $4}')
Exp1=$(curl https://raw.githubusercontent.com/MyMasWayVPN/mymaswayvpn.github.io/main/akses/ip/buyer/izin | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
...
clear
# REPO    
      REPONYA="https://abc.xcodehoster.com/"
#   REPOKU="https://myridtunnel.my.id/"
      REPO="https://raw.githubusercontent.com/MyMasWayVPN/vi/main/"
      REPOKU="https://raw.githubusercontent.com/MyMasWayVPN/vi/main/"
####
start=$(date +%s)
secs_to_human() {
...
rm -rf /etc/vmess/.vmess.db
    ...
    mkdir -p /etc/myrid/limit/vmess/ip
    mkdir -p /etc/myrid/limit/vless/ip
    mkdir -p /etc/myrid/limit/trojan/ip
    mkdir -p /etc/myrid/limit/ssh/ip
...
bash -c "$(curl -L https://github.com/XTLS/Xray-install/raw/main/install-release.sh)" @ install -u www-data --version $latest_version
    ...
    # // Ambil Config Server
    wget -O /etc/xray/config.json "${REPONYA}xray/config.json" >/dev/null 2>&1
    #wget -O /usr/local/bin/xray "${REPONYA}xray/xray.linux.64bit" >/dev/null 2>&1
    wget -O /etc/systemd/system/runn.service "${REPONYA}xray/runn.service" >/dev/null 2>&1
    ...
    wget -O /etc/haproxy/haproxy.cfg "${REPONYA}xray/haproxy.cfg" >/dev/null 2>&1
    wget -O /etc/nginx/conf.d/xray.conf "${REPONYA}xray/xray.conf" >/dev/null 2>&1
    sed -i "s/xxx/${domain}/g" /etc/haproxy/haproxy.cfg
    sed -i "s/xxx/${domain}/g" /etc/nginx/conf.d/xray.conf
    curl ${REPONYA}ssh/nginx.conf > /etc/nginx/nginx.conf
...
print_install "Memasang Password SSH"
    wget -O /etc/pam.d/common-password "${REPONYA}ssh/password"
...
wget -q -O /usr/local/kyt/udp-mini "${REPONYA}badvpn/udp-mini"
chmod +x /usr/local/kyt/udp-mini
wget -q -O /etc/systemd/system/udp-mini-1.service "${REPONYA}badvpn/udp-mini-1.service"
wget -q -O /etc/systemd/system/udp-mini-2.service "${REPONYA}badvpn/udp-mini-2.service"
wget -q -O /etc/systemd/system/udp-mini-3.service "${REPONYA}badvpn/udp-mini-3.service"
...
domain=$(cat /etc/xray/domain)
    ...
    ◇━━━━━━━━━━━━━━━━━━━━━━━◇
    ⚠︎𝘽𝙮 MyridTunnel 𝙋𝙧𝙤𝙟𝙚𝙘𝙩 𝙈𝙖𝙣𝙖𝙜𝙚𝙧 𝘾𝙤𝙢𝙢𝙪𝙣𝙞𝙩𝙮⚠︎
    ◇━━━━━━━━━━━━━━━━━━━━━━━◇
"
...
echo "    ============================" | tee -a log-install.txt
echo "  ⚠︎𝘽𝙮 MyridTunnel 𝙋𝙧𝙤𝙟𝙚𝙘𝙩 𝙈𝙖𝙣𝙖𝙜𝙚𝙧 𝘾𝙤𝙢𝙢𝙪𝙣𝙞𝙩𝙮⚠︎" | tee -a log-install.txt
echo "    ============================" | tee -a log-install.txt
echo ""
